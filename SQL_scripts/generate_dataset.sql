...
DROP PROCEDURE IF EXISTS generate_encounters_micro;
DELIMITER //
CREATE PROCEDURE generate_encounters_micro(IN p_total INT, IN p_batch INT)
BEGIN
    DECLARE inserted INT DEFAULT 0;
    DECLARE rows_this_batch INT;
    DECLARE start_id INT;

    -- Build FK-safe temporary indexes (Section 0 above)
    DROP TEMPORARY TABLE IF EXISTS tmp_patient_index;
    CREATE TEMPORARY TABLE tmp_patient_index (rn INT PRIMARY KEY, patient_id INT) ENGINE=MEMORY;
    SET @rn := 0;
    INSERT INTO tmp_patient_index (rn, patient_id)
    SELECT (@rn := @rn + 1), patient_id FROM patients ORDER BY patient_id;

    DROP TEMPORARY TABLE IF EXISTS tmp_provider_index;
    CREATE TEMPORARY TABLE tmp_provider_index (rn INT PRIMARY KEY, provider_id INT, department_id INT) ENGINE=MEMORY;
    SET @rn := 0;
    INSERT INTO tmp_provider_index (rn, provider_id, department_id)
    SELECT (@rn := @rn + 1), provider_id, department_id FROM providers ORDER BY provider_id;

    SET @patient_count  := (SELECT COUNT(*) FROM tmp_patient_index);
    SET @provider_count := (SELECT COUNT(*) FROM tmp_provider_index);

    SELECT COALESCE(MAX(encounter_id), 0) + 1 INTO start_id FROM encounters;

    WHILE inserted < p_total DO
        SET rows_this_batch = LEAST(p_batch, p_total - inserted);
        START TRANSACTION;

        WITH numbers AS (
            -- n = 0 .. rows_this_batch-1
            ...
            WHERE a.N + b.N*10 + c.N*100 + d.N*1000 + e.N*10000 < rows_this_batch
        ),
        flags AS (
            SELECT n, FLOOR(1 + RAND()*3) AS enc_flag
            FROM numbers
        )
        INSERT INTO encounters (encounter_id, patient_id, provider_id, encounter_type, encounter_date, discharge_date, department_id)
        SELECT
            start_id + inserted + f.n AS encounter_id,
            p.patient_id,
            pr.provider_id,
            ELT(f.enc_flag, 'Outpatient', 'Inpatient', 'Emergency') AS encounter_type,
            DATE_SUB(CURDATE(), INTERVAL FLOOR(RAND() * 1095) DAY) + INTERVAL FLOOR(RAND() * 24) HOUR AS encounter_date,
            CASE WHEN f.enc_flag = 2 THEN
                DATE_SUB(CURDATE(), INTERVAL FLOOR(RAND() * 1095) DAY) + INTERVAL FLOOR(RAND() * 24) HOUR + INTERVAL FLOOR(1 + RAND() * 14) DAY
            ELSE NULL END AS discharge_date,
            pr.department_id
        FROM flags f
        JOIN tmp_patient_index  p  ON p.rn  = 1 + FLOOR(RAND() * @patient_count)
        JOIN tmp_provider_index pr ON pr.rn = 1 + FLOOR(RAND() * @provider_count);

        COMMIT;
        SET inserted = inserted + rows_this_batch;
        SELECT CONCAT('Encounters: ', inserted, ' inserted') AS progress;
        -- Optional tiny pause to reduce contention
        SELECT SLEEP(0.1);
    END WHILE;
END//
DELIMITER ;

-- Run: 500k in batches of 50k
CALL generate_encounters_micro(500000, 50000);
